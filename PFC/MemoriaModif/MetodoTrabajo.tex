...
El Proceso Unificado de Desarrollo es una evolución del Proceso Unificado de Rational (RUP), que define un ``\textit{conjunto de actividades necesarias para transformar los requisitos de usuario en un sistema software}'' \cite{jac00}. Más concretamente se puede definir como ``\textit{un marco genérico de trabajo que puede especializarse para una gran variedad de sistemas de software,
...
\imagenBorde{Cap4//incremental}{0.50}{Proceso iterativo e incremental en el PUD \cite{jacobson1999unified}}{fig:incremental}
...
\imagenBorde{Cap4//PUD}{1.0}{Fases y flujo de trabajo del PUD \cite{jac00}}{fig:PUD}
...
\begin{tabular}{| >{\centering\arraybackslash}m{2cm} | >{\arraybackslash}m{7cm} | >{\arraybackslash}m{5cm} |}
	\hline
		\multicolumn{1}{|c|}{\colorCabecera\textbf{Fase}} &
			\multicolumn{1}{|c|}{\colorCabecera\textbf{Resumen}} &
		\multicolumn{1}{|c|}{\colorCabecera\textbf{Artefactos generados}} \\
	\hline
...
Subversion es un sistema de control de versiones donde, a diferencia de CVS (Concurrent Versions System), los archivos versionados del repositorio no tienen cada uno un número de versión distinto, sino que todo el repositorio tiene un único numero de versión. Además, Subversion permite acceder a repositorios de red.
...
Se utilizará esta herramienta, en su versión 2.2.1, para la gestión del proyecto y sus dependencias con otras librerías y módulos, usando el plugin de Maven para Eclipse llamado \textbf{m2eclipse}. 
...
Es el entorno de desarrollo utilizado para la codificación del proyecto utilizando el lenguaje de programación Java, junto a plugins utilizados para la gestión de proyectos con Maven (plugin \textbf{m2eclipse}) y diseño de interfaces gráficas con Swing (plugin \textbf{Jigloo}). 

\subsubsection{Yahoo! PlaceFinder} \label{label:web}

Yahoo! PlaceFinder es un servicio web de geocodificación que permite la conversión de direcciones de calles o nombres de lugares en coordenadas geográficas, y viceversa. La respuesta a una petición de este servicio Web viene dada por un fichero XML.
...
\imagenBorde{Cap4//jaxb}{0.9}{JAXB: \textit{Java Architecture for XML Binding}}{fig:jaxb}
...
Swing es una librería gráfica para Java, la cual incluye \textit{widgets} para generar interfaces gráficas de usuario, como son cajas de texto, botones, paneles, tablas, etc. Swing fue desarrollado para proveer un conjunto de elementos gráficos más sofisticado que su antecesor, AWT (\textit{Abstract Window Toolkit}), de tal modo que los componentes Swing están escritos en Java y son independientes de la plataforma, a diferencia de AWT.

Swing sigue el modelo vista controlador (MVC), es independiente de la plataforma y extensible, ya que los componentes pueden extenderse y sobrescribir las implementaciones por defecto.

Existe una extensión a Swing, llamada \textbf{SwingX}, que incluye nuevos componentes gráficos que normalmente se demandan en aplicaciones avanzadas (Rich Client Applications), como son selectores de fechas, framework para la autenticación de usuarios, auto-completado, paneles expandibles, etc.

Por otra parte, existe un framework para Swing, llamado \textbf{Swing Application Framework}, que forma parte de la JSR 296 (Java Specification Request) y que permite gestionar la arquitectura, ciclo de vida, hilos, manejo de eventos y almacenamiento del estado de una aplicación Swing.
...
\begin{itemize}
	...
	\item Soporte para diferentes tipos de salida, como componentes de Swing, imágenes (PNG y JPEG) y gráficos vectoriales (EPS y SVG)
...
JUnit es un framework que permite la ejecución de casos de prueba unitarios para evaluar si los diferentes métodos e instrucciones de las clases de Java se comportan según lo esperado. Este framework permite visualizar los resultados de las pruebas en modo texto, gráfico o como una tarea de \textit{Ant}.

Se utilizará JUnit, como un plug-in de Eclipse en su versión 4.8.1 para ejecutar las pruebas del sistema.


\subsubsection{Eclemma}

Eclemma es una herramienta gratuita de cobertura de código Java que permite analizar y visualizar las instrucciones, métodos y bloques cubiertos por un conjunto de pruebas. Además, permite exportar los resultados a informes HTML, a ficheros CSV o a ficheros XML.

Se ha utilizado Eclemma, en forma de plugin para Eclipse en su versión 2.0.1, para medir la cobertura de código alcanzada con las pruebas realizadas en el sistema.
...
Para la generación del presente documento, se ha utilizado \LaTeX{}, usando como editor de textos \textbf{TexnicCenter}, el cual es un editor de software libre para sistemas Windows que proporciona las herramientas necesarias para la composición y compilación de textos escritos en \LaTeX{}.
...
Se ha utilizado \BibTeX{} para formatear las citas literarias y bibliográficas de este documento. Además, se ha utilizado \textbf{JabRef}, que es una herramienta visual basada en Java que permite editar los tipos bibliográficos utilizados en el archivo \textit{.bib} de \BibTeX{}.

\subsubsection{GIMP}

GIMP (GNU Image Manipulation Program) es una aplicación diseñada para la manipulación y edición de imágenes, con una potencia similar a Adobe Photoshop, siendo de código abierto, gratuita y multiplataforma.

Se ha utilizado esta herramienta para la generación y edición de las imágenes utilizadas en la documentación y en el sistema software desarrollado.
