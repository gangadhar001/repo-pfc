...
\begin{tabular}{| >{\centering\arraybackslash}m{5.5cm} | >{\arraybackslash}m{6cm} | >{\centering\arraybackslash}m{2cm} |}
	...
	\hline
		\multirow{1}{*}{\colorCelda \textbf{F3:} Visualización información} & 
		 ...
		 \colorCelda & Visualizar compañía & \textbf{F3.2} \\ 
	\hline
...
Los actores que aparecen en los diagramas de casos de uso del cliente se refieren a las personas físicas que pueden utilizar el sistema (empleado y jefe de proyecto) y al subsistema servidor. Por otro lado, en el servidor aparece como actor el subsistema cliente y el actor que representa la base de datos. Se ha decidido modelar estos actores que representan sistemas software (cliente, servidor y bases de datos) para representar visualmente las interacciones que ocurren entre los sistemas, ya que algunos casos de uso no interaccionan, por ejemplo, con el sistema de base de datos o con el cliente.
...
Se elabora un glosario de términos para describir algunos de los conceptos utilizados a lo largo de la elaboración del presente PFC, para eliminar posibles ambigüedades.

\begin{itemize}
	\item \textbf{Compañía}: el término \textit{compañía} en el contexto de este PFC hace referencia a una empresa u organización dedicada y/o involucrada al desarrollo de productos software.
...
Uno de los puntos más importantes detectados en este análisis previo es la necesidad de crear un sistema cliente-servidor, donde el cliente sea totalmente independiente de la implementación del servidor. Además, otro aspecto destacado es que la interfaz gráfica del cliente debe ser flexible y adaptable a las operaciones que los diferentes usuarios puedan realizar en el sistema.

Analizando las tecnologías existentes, se comprobó que era factible realizar estos objetivos destacados, gracias al uso de \textbf{RMI} y del API de reflexión de Java. Además, con el resto de tecnologías utilizadas, se podía cubrir el resto de objetivos del sistema.
