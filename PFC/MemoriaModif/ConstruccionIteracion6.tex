...
Siguiendo los casos de uso del grupo funcional \textbf{F5:} \textit{Gestión de proyectos} (ver Figura \ref{fig:CduGestionProyectos2Server}), así como el análisis realizado, se abordan las siguientes tareas en esta iteración:
...
También es importante destacar el caso de uso \textit{Aconsejar Decisiones}, ya que éste es otro de los requisitos más importantes del sistema. Así, en la Figura \ref{fig:secuenciaCBRCliente} se muestra el diagrama de secuencia para este caso de uso en el cliente, mientras que en la Figura \ref{fig:secuenciaCBRServer} se observa el diagrama de secuencia para el servidor.
...
Hay que señalar que en esta funcionalidad se incluye el comportamiento de los casos de uso \textit{Consultar decisiones} y \textit{Consultar proyectos}, por lo que dichos comportamientos se muestran de manera simplificada en estos diagramas de secuencia.
...
\begin{figure}[!htbp]
 \begin{center}
   \includegraphics[scale=1.85,angle=90]{Cap5/Server2//clasesCBR}
...
Respecto al subsistema cliente, tiene especial interés el diseño e implementación del caso de uso \textit{Solicitar recomendaciones}. A continuación se comentan algunos detalles tenidos en cuenta a la hora del diseño y la implementación, ya que en el Apéndice \ref{appendix:manualCliente} se muestra paso a paso el proceso que hay que seguir para poder crear la configuración necesaria para ejecutar el CBR.
